...
class ecElementBase(Structure):
    ...
    def getPoint(self):
        point = {'x':"",'y':""}
        d = self._str_degree_max if self._str_degree_max else self._degree
        points = [p for p in self.points[:d] ]
        # print(points[0].x)
        if self.normalized:
            fmt = "\tx:{}\n\ty:{}"
        ...
        return list(map(hexString, coords))

    def getX(self):
        return relicResult(librelic.ec_get_x_abi,  self)
...
class ecPoint(Structure):
    ...
    _fields_ = [
                ...
                # fixed length.
                ("x", c_ulonglong*int(COORD_LEN)), 
                ("y", c_ulonglong*int(COORD_LEN)),
                ("z", c_ulonglong*int(COORD_LEN))
            ]
...
class ec1Element(ecElementBase):
    ...
    def __sub__(self, other):
        #Subtracts an elliptic curve point 
        return relicResult(librelic.ec_sub_abi, ec1Element, self, other)
...
def ecmap(x):
    ...
    result = ec1Element()
    librelic.ec_map_abi(byref(result), byref(b), len(x))
    return result
...
def _equal(a, b, identityLong, relicCompare):
    ...
    if isinstance(b, (int, int)) and b == identityLong:
        return a.isIdentity()
